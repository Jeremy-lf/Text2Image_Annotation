...
# 用于销毁当前进程组。在进行分布式训练时，会创建一个进程组来管理多个进程之间的通信。
# 当训练结束时，需要调用此函数来清理资源，确保所有进程正常退出，避免资源泄漏或僵尸进程。
def cleanup():
    """
    End DDP training.
    """
...
def create_logger(logging_dir):
    ...
    if dist.get_rank() == 0:  # real logger
        ...
        logger = logging.getLogger(__name__)
        logger.addHandler(logging.NullHandler()) # NullHandler，这意味着所有通过这个日志记录器发送的日志消息都将被忽略，不会输出到任何地方。
    return logger
...
def setup_lm_ddp_sync(model: nn.Module) -> DDP:
    # 将模型包装为 DDP
    model = DDP(
        model,
        device_ids=[torch.cuda.current_device()],  # 当前 GPU 设备
    )
    torch.cuda.synchronize()
    return model
...
def main(args):
    ...
    print(f"Procedp_ss {dp_rank} initialized successfully.")
    rank = dist.get_rank()
    ...
    vae = AutoencoderKL.from_pretrained("/root/paddlejob/workspace/env_run/output/lvfeng/model/vae", torch_dtype=torch.bfloat16).to(
        device
    )
    ...
    for train_res in [1024]:
        ...
        image_transform = transforms.Compose(
            [
                # var_center_crop函数的作用是从一组可能的裁剪尺寸中选择一个最适合图像尺寸的裁剪尺寸（
                # 通过比较裁剪尺寸与图像尺寸的宽高比来确定“最适合”），并使用这个裁剪尺寸对图像进行中心裁剪。
    ...
    for step in range(resume_step, args.max_steps):
        ...
        for train_res, data_pack in data_collection.items():
            ...
            # 下面开始对batch内的字段信息进行转换处理, data_items: [{'image_path':xx, 'prompt:',yy},{...}], group_names:['text_image_pair']
            x, caps, cond, position_type = [], [], [], []
            for data_item, group_name in zip(data_items, group_names):
                if group_name == "text_image_pair":
                    # inpaint image
                    x_, caps_, cond_ = t2i_item_processor.text_image_pair_processor(data_item, "infilling")
                    x.append(x_)
                    caps.append(caps_) #[caption1, caption2,...]
                    cond.append(cond_)
            ...
            x = [img.to(device, non_blocking=True) for img in x] # [img1,img2, img3,...]
            cond = [[cond_img.to(device, non_blocking=True) for cond_img in cond_imgs] for cond_imgs in cond] # [[cond_img1,cond_img2, cond_img3,...]]
            ...
            with torch.no_grad():
                ...
                for i, img in enumerate(x):
                    x[i] = (vae.encode(img[None].bfloat16()).latent_dist.mode()[0] - vae_shift) * vae_scale  # latent features
                    x[i] = x[i].float()
            ...
            with torch.no_grad():
                cap_feats, cap_mask = encode_prompt(caps, text_encoder, tokenizer, args.caption_dropout_prob) # 对文本进行编码，返回特征和mask
            ...
            # 需要搞清楚这一块的local_bsz和micro_bsz的关系
            for mb_idx in range((data_pack["local_bsz"] - 1) // data_pack["micro_bsz"] + 1):
                mb_st = mb_idx * data_pack["micro_bsz"] # value=0
                mb_ed = min((mb_idx + 1) * data_pack["micro_bsz"], data_pack["local_bsz"]) # value=1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--master_port", type=int, default=8020)
