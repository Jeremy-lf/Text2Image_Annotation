...
def process_image_mask_v3(paint, mode, aspect_ratio):
    if mode == "Image Infilling":
        image, mask = paint['background'], paint['layers'][0].convert('L')
        ...
        image = np.zeros((height, width, 3), dtype=np.uint8) + 255  # White image
        mask = np.zeros((height, width), dtype=np.uint8)  # Black mask
        print("Generated image:", image.shape, "Generated mask:", mask.shape)
...
def load_models(args):
    ...
    # lora
    # ckpt_path="model/sft/plain_model.pt"
    # ckpt_path = "lumina_results/checkpoints/0044000/consolidated.00-of-01.pth"
    # ckpt_path = "lumina_results_cheduan/checkpoints/0020000/consolidated.00-of-01.pth"
    # ckpt_path = "lumina_results_cheduan_patch_weight/checkpoints/0045000/consolidated.00-of-01.pth"
    # ckpt_path = "lumina_results_cheduan_patch_weight/checkpoints/0070000/consolidated.00-of-01.pth"
    # ckpt_path = "lumina_results_cheduan_patch_weight/checkpoints/0055000/consolidated.00-of-01.pth"
    # ckpt_path = "lumina_results_cheduan_patch_weight/checkpoints/0090000/consolidated.00-of-01.pth"
    # ckpt_path = "lumina_results_cheduan_patch_weight/checkpoints/0100000/consolidated_ema.00-of-01.pth"
    ckpt_path = "lumina_results_cheduan_patch_weight/checkpoints/0105000/consolidated.00-of-01.pth"
    # ckpt_path = "lumina_results_cheduan_luce_patch_weight/checkpoints/0020000/consolidated.00-of-01.pth"
    # ckpt_path = "lumina_results_cheduan_luce_patch_weight/checkpoints/0030000/consolidated.00-of-01.pth"
...
def create_demo():
    ...
    with gr.Blocks() as demo:
        gr.Markdown("# Traffic AIGC Model")
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Row():
                    mode_dropdown = gr.Dropdown(["Image Infilling", "Text-to-Image"], label="Mode")
                    aspect_ratio_dropdown = gr.Dropdown(["1920x1080", "2560x1920", "2592x2048"], label="Aspect Ratio", visible=False)

                def update_visibility(mode):
                    return gr.update(visible=(mode == "Text-to-Image"))

                mode_dropdown.change(fn=update_visibility, inputs=[mode_dropdown], outputs=[aspect_ratio_dropdown])
                ...
                with gr.Row():
                    ...
                    seed = gr.Number(label="Random Seed (leave empty for random)", precision=0, value=20230122)
            ...
            generate_btn1.click(fn=process_image_mask_v3, inputs=[cond_image, mode_dropdown, aspect_ratio_dropdown], outputs=[condition_preview])
        ...
        generate_btn.click(
            ...
            inputs=[condition_preview, prompt, mode_dropdown,
        ...
        # 定义三个示例按钮
        with gr.Row():
            example1_btn = gr.Button("Example 1: Text-to-Image(Car View)")
            example2_btn = gr.Button("Example 2: Image Infilling(Car View)")
            example3_btn = gr.Button("Example 3: Text-to-Image(Road View)")
        
        # 定义示例数据
        example_data = [
            [
                "examples/text2image.jpg",
                "这是一幅高速公路场景的图片。天气晴朗，天空清澈。图片左侧有一辆白色的小轿车，前方有一个红色卡车。右侧是一辆大型绿色篷布覆盖的货车。道路上有三条清晰的车道划分线，标示了直行方向。远处的路牌有绿色的路标指示牌。路面干燥，没有积水或湿滑现象。道路边缘有护栏，环境整洁有序。",
            ],
            [
                "examples/image_origin.jpg",
                "前方道路上发生了火灾，并且伴有烟雾，火势很大",
                "examples/image_condition.png",
                "examples/image_fire.png",
            ]
        ]
        
        # 为每个按钮设置点击事件
        def load_example_image_infilling(example_idx):
            example = example_data[example_idx]
            return [
                example[0],  # input_image
                example[1],  # prompt
                example[2],  # condition
                example[3]   # output
            ]
        
        def load_example_text2image(example_idx):
            example = example_data[example_idx]
            return [
                example[0],  # output_image
                example[1],  # prompt
            ]
        
        example1_btn.click(
            fn=lambda: load_example_text2image(0),
            inputs=[],
            outputs=[
                output_image, prompt,
            ]
        ...
        example2_btn.click(
            fn=lambda: load_example_image_infilling(1),
            inputs=[],
            outputs=[
                cond_image, prompt, condition_preview, output_image
            ]
        )
