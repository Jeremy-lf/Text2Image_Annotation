...
def process_image_mask_v2(paint) :
    ...
    image, mask = np.array(image), np.array(mask) # mask: wxh
...
def create_demo():
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                cond_image = gr.ImageEditor(label='Input', type='pil', image_mode='RGB', sources=['upload'], layers=False,
                ...
                # 用于选择是否生成条件图像。如果选中，会显示一个 gr.Dropdown 组件来选择条件类型（如“Canny Edge”）。
                ...
                with gr.Row():
                    resolution = gr.Slider(minimum=512, maximum=1024, value=512, step=256, label="Inference Resolution")
            ...
            with gr.Column():
                with gr.Row():
                    condition_preview = gr.Image(label="Condition Preview", visible=True, format="jpg")
        ...
        def preview_condition(img, cond_type):
            ...
            if cond_type == "Image Infilling":
                cond_img = process_image_mask_v2(img)
            else:
                return None
        ...
        def process_and_generate(img, txt, sys_prompt, gen_cond, cond_type, steps, scale, t_shift_val, seed_val, resolution):
            if img is None:
            ...
            if gen_cond:
                if cond_type == "Image Infilling":
                    cond_img = process_image_mask_v2(img)
                    sys_prompt_key = "Image Infilling"
                else:
        ...
        generate_btn.click(
            ...
            inputs=[condition_preview, prompt, system_prompt_dropdown, generate_condition, condition_type, 
