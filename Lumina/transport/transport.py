...
"""
定义了一个名为 ModelType 的枚举（enum）类，它用于表示模型预测的输出类型。枚举是一种特殊的类，用于表示一组常量值。
"""
class ModelType(enum.Enum):
    """
    Which type of output the model predicts.
    enum.auto() 是一个特殊的枚举值，它会自动为每个枚举成员分配一个唯一的整数值。
    """
...
class Transport:
    ...
    def sample(self, x1):
        ...
        if self.snr_type.startswith("uniform"):
            ...
            # 通过变换正态分布采样得到的u来计算t，使得t的分布接近对数正态分布，并且值被限制在[t0, t1]区间内。
        else:
