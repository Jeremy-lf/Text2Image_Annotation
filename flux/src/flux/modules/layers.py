...
"""
EmbedND类实现了一个具有特定嵌入逻辑和可能的旋转或位置编码机制的神经网络模块。
它通过rope函数对输入的每个轴进行某种变换，然后将这些变换后的结果拼接起来，并在特定维度上增加一个额外的维度作为输出。
这个类可以用于需要将输入数据嵌入到更高维空间，并对数据的不同轴进行特定变换的场景中。
"""
...
def timestep_embedding(t: Tensor, dim, max_period=10000, time_factor: float = 1000.0):
    ...
    """
    # t: 时间步张量，形状为(N,)，值可为浮点数（如0~1之间的归一化时间）
    # dim: 输出维度（需为偶数，若为奇数则自动补零）
    # max_period: 控制最小频率（周期），值越大频率越低
    # time_factor: 时间缩放因子，扩大时间范围以增加频率分辨率
    t = time_factor * t  # N纬度
    half = dim // 2
    ...
    embedding = torch.cat([torch.cos(args), torch.sin(args)], dim=-1)  # 得到形状为(N, dim)的嵌入张量embedding
    if dim % 2:
        # 如果dim是奇数，则embedding的维度会少一个，此时需要在最后拼接一个与embedding第一列形状相同的零张量，以确保输出维度正确
...
"""
SingleStreamBlockLoraProcessor 类通过结合LoRA技术和注意力机制，实现了一种高效的模型调整方法。
LoRA技术通过低秩分解减少了需要调整的参数数量，使得在保持模型性能的同时，能够更高效地适应新任务。
这种方法在大型预训练模型的微调中特别有用，可以显著减少计算资源和存储需求。
"""
class SingleStreamBlockLoraProcessor(nn.Module):
    def __init__(self, dim: int, rank: int = 4, network_alpha = None, lora_weight: float = 1):
        super().__init__()
        self.qkv_lora = LoRALinearLayer(dim, dim * 3, rank, network_alpha)  # up and down linear layers
...
class LastLayer(nn.Module):
    def __init__(self, hidden_size: int, patch_size: int, out_channels: int):
        super().__init__()
        self.norm_final = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)  # eps=1e-6是一个很小的数，用于防止在归一化过程中分母为零。
        self.linear = nn.Linear(hidden_size, patch_size * patch_size * out_channels, bias=True)  # 特征到像素空间的投影层
        self.adaLN_modulation = nn.Sequential(nn.SiLU(), nn.Linear(hidden_size, 2 * hidden_size, bias=True))  # 自适应归一化层

    def forward(self, x: Tensor, vec: Tensor) -> Tensor:
        shift, scale = self.adaLN_modulation(vec).chunk(2, dim=1)  # 将条件调制拆分shift和scale, vec:Bx768 shift/scale:Bx768
        x = (1 + scale[:, None, :]) * self.norm_final(x) + shift[:, None, :]  # x:BxLx768 自适应LayerNorm
        x = self.linear(x) #BxLx(16*16*3)
        return x
